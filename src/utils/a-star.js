 ...
 * 
 * Code modified by Ítalo Nascimento to be used within the Closure Tools lib.
 */

goog.provide('lpc.utils.AStar');

lpc.utils.AStar = function () {}

lpc.utils.AStar.prototype.diagonalSuccessors = function($N, $S, $E, $W, N, S, E, W, grid, rows, cols, result, i) {
    if($N) {
...
lpc.utils.AStar.prototype.diagonalSuccessorsFree = function($N, $S, $E, $W, N, S, E, W, grid, rows, cols, result, i) {
    $N = N > -1;
    $S = S < rows;
    $E = E < cols;
    $W = W > -1;
...
lpc.utils.AStar.prototype.nothingToDo = function($N, $S, $E, $W, N, S, E, W, grid, rows, cols, result, i) {
    return result;
}

lpc.utils.AStar.prototype.successors = function(find, x, y, grid, rows, cols){
    var
        N = y - 1,
        S = y + 1,
        E = x + 1,
...
lpc.utils.AStar.prototype.diagonal = function(start, end, f1, f2) {
    return f2(f1(start.x - end.x), f1(start.y - end.y));
}

lpc.utils.AStar.prototype.euclidean = function(start, end, f1, f2) {
    var
        x = start.x - end.x,
        y = start.y - end.y
    ;
...
lpc.utils.AStar.prototype.manhattan = function(start, end, f1, f2) {
    return f1(start.x - end.x) + f1(start.y - end.y);
}

lpc.utils.AStar.prototype.AStar = function(grid, start, end, f) {
    var
        cols = grid[0].length,
        rows = grid.length,
        limit = cols * rows,
